...
export function GuidedPromptBuilder({ onPromptChange, onGenerate, isGenerating = false, onUploadClick, hasAttachment = false }: GuidedPromptBuilderProps) {
  ...
  useEffect(() => {
    if (!isGenerating) { loadingIdx.current = 0; setLoadingText(LOADING_PHRASES[0]); return; }
    const iv = setInterval(() => {
      loadingIdx.current = (loadingIdx.current + 1) % LOADING_PHRASES.length;
      setLoadingText(LOADING_PHRASES[loadingIdx.current]);
    }, 4000);
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="fixed bottom-0 left-0 right-0 z-40 px-4 py-3 bg-[#0a0a0a]/95 backdrop-blur-md border-t border-border/40 flex items-center justify-between gap-3 sm:static sm:px-0 sm:py-0 sm:bg-transparent sm:backdrop-blur-none sm:border-t-0">
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            >
              {isGenerating ? "Building your course..." : "Generate course"}
            </button>
            {isGenerating && (
              <span className="text-[11px] text-muted-foreground animate-pulse">{loadingText}</span>
            )}
          </div>
        </div>
      </div>
